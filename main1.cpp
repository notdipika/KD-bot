...
private:
    ...
    bool parseFunction(const string& input, const string& func, double& operand1, double& operand2) {
        ...
        size_t end = input.find(')', comma);
        if (comma == string::npos || end == string::npos) return false;
        string number1Str = input.substr(start, comma - start);
        string number2Str = input.substr(comma + 1, end - comma - 1);
        stringstream ss1(number1Str);
        stringstream ss2(number2Str);
        return (ss1 >> operand1) && (ss2 >> operand2) ? true : false;
    }
    ...
    bool parseFunction(const string& input, const string& func, double& operand1, double& operand2, double& operand3) {
        ...
        size_t end = input.find(')', comma2);
        if (comma1 == string::npos || comma2 == string::npos || end == string::npos) return false;
        string number1Str = input.substr(start, comma1 - start);
        ...
        return (ss1 >> operand1) && (ss2 >> operand2) && (ss3 >> operand3) ? true : false;
    }
    ...
    bool parseFunction(const string& input, const string& func, double& real1, double& imag1, double& real2, double& imag2) {
        ...
        size_t end = input.find(')', comma3);
        if (comma1 == string::npos || comma2 == string::npos || comma3 == string::npos || end == string::npos) return false;
        string real1Str = input.substr(start, comma1 - start);
        string imag1Str = input.substr(comma1 + 1, comma2 - comma1 - 1);
        string real2Str = input.substr(comma2 + 1, comma3 - comma2 - 1);
        string imag2Str = input.substr(comma3 + 1, end - comma3 - 1);
        stringstream ss1(real1Str);
        stringstream ss2(imag1Str);
        stringstream ss3(real2Str);
        stringstream ss4(imag2Str);
        return (ss1 >> real1) && (ss2 >> imag1) && (ss3 >> real2) && (ss4 >> imag2) ? true : false;
    }

    bool parseMatrix(const string& input) {
        // Placeholder for parsing matrix from string input
        return true;
    }
        ...
        while (true) {
            if (loggedInUser.empty()) {
                ...
                switch (choice) {
                    ...
                    case 2: {
                        string conversation;
                        cout << "Enter conversation to save: ";
                        getline(cin, conversation);
                        cm.saveConversation(loggedInUser, conversation);
                        break;
                    }
                    case 3:
                        cm.loadConversation(loggedInUser);
                        break;
                    case 4:
                        cm.deleteConversation(loggedInUser);
                        break;
                    case 5:
                        loggedInUser.clear();
